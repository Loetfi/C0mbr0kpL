...
/* ==========================================================
  Gmap
=========================================================== */
if ( !function_exists('havnor_gmap_function')) {
  function havnor_gmap_function( $atts, $content = NULL ) {
    ...
    if ($gmap_common_marker) {
      ...
      $common_marker = HAVNOR_PLUGIN_ASTS . '/images/map-marker.png';
    }
    ...
    // Google Map Height
    if ( $gmap_height ) {
      $inline_style .= '.hanor-map-'. $e_uniqid .' {';
      $inline_style .= ( $gmap_height ) ? 'height:'. havnor_core_check_px($gmap_height) .';' : '';
      $inline_style .= '}';
    }

    // add inline style
    add_inline_style( $inline_style );
    $styled_class  = ' hanor-map-'. $e_uniqid;

    wp_enqueue_script( 'vtheme-googlemap', HAVNOR_PLUGIN_ASTS . '/jquery.googlemap.js', array( 'jquery' ), '1.5.0', true );
    ...
    // Map Tab ID
    if ($gmap_id) {
      $gmap_open = '<div '. $gmap_id .' class="hanor-gmap-tab tab-pane fade">';
      $gmap_close = '</div>';
    } else {
      $gmap_open = '';
      $gmap_close = '';
    ...
    echo $gmap_open;
    echo '<div id="map-'. $e_uniqid .'" class="hanor-map '. $custom_css . $class . $styled_class .'"></div>';
    echo $gmap_close;
...
?>
    ...
    jQuery(document).ready(function($) {
      ...
      <?php foreach ( $locations as $location ) {
        ...
        $text = (isset($location['location_text'])) ? 'text: "<p>'. $location['location_text'] .'</p>",' : '';
      ?>
...
add_shortcode( 'hanor_gmap', 'havnor_gmap_function' );
